...
def load_odise_model():
    ...
    print("\n\n")
    print("thing_classes ",len(demo_metadata.thing_classes),demo_metadata.thing_classes)
    
    print("\n\n")
    print("stuff_classes ",len(demo_metadata.stuff_classes),demo_metadata.stuff_classes)

    print("\n\n")
    print("thing_colors ",len(demo_metadata.thing_colors),demo_metadata.thing_colors)

    print("\n\n")
    print("stuff_colors ",len(demo_metadata.stuff_colors),demo_metadata.stuff_colors)

    print("\n\n")
    print("thing_dataset_id_to_contiguous_id ",len(demo_metadata.thing_dataset_id_to_contiguous_id),demo_metadata.thing_dataset_id_to_contiguous_id)

    print("\n\n")
    print("stuff_dataset_id_to_contiguous_id ",len(demo_metadata.stuff_dataset_id_to_contiguous_id),demo_metadata.stuff_dataset_id_to_contiguous_id)


    print("*"*50)
    print("Loaded Model")
    print("*"*50)
    return inference_model,demo_metadata, aug,demo_metadata.stuff_classes,demo_metadata.stuff_colors

#laoding the model
inference_model,demo_metadata, aug, class_names, colors = load_odise_model()

def add_class_names_to_segments_info(segments_info):
    global class_names
    # Iterate through segments_info and add the corresponding class name to each entry
    for idx, segment_info in enumerate(segments_info):
        if idx < len(class_names):
            segment_info['class_name'] = class_names[idx]
...
def predict(img,output_filename):
  with ExitStack() as stack:
        ...
        print("\n\n\n")
        print(panoptic_seg.shape,panoptic_seg)
        print("\n\n\n")
        print(segments_info)
        print("\n\n\n")
        print(add_class_names_to_segments_info(segments_info))
        print("\n\n\n")
...
############ Demo of odise on local image without ds ###################
# input_image = image = utils.read_image("demo/examples/coco.jpg", format="RGB")
# predict(input_image,"demo/output/pred_output.jpg")
########################################################################
...
def main(args):
    ...
    filesink.set_property("location", "demo/output/output.jpg")
    ...
    if not is_aarch64():
        ...
        nvvidconv3.set_property("nvbuf-memory-type", mem_type)
    ...
    source.link(decoder)
    decoder.link(nvvidconv3)

    sinkpad = streammux.get_request_pad("sink_0")
    if not sinkpad:
        sys.stderr.write(" Unable to get the sink pad of streammux \n")
    srcpad = nvvidconv3.get_static_pad("src")
    if not srcpad:
        sys.stderr.write(" Unable to get source pad of nvvidconv3 \n")
    srcpad.link(sinkpad)
